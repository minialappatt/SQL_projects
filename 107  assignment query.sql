...
select * from programmer where salary=max(salary) group by developin

--48. Who is the least experienced Programmer.
select * from (select min(datediff(year,doj,getdate())) as experience from programmer) as test

--49. Who is the most experienced male programmer knowing PASCAL.
select MAX(datediff(year,doj,getdate())) AS experience from programmer 
...
select s.PNAME from studies st,software s where COURSE_FEE in (select MIN(course_fee) from software)
...
select s.PNAME, st.INSTITUTE  from software s,studies st where DCOST=(select MIN(dcost) from software)

select st.institute, COUNT(s.TITLE) from software s,studies st where s.PNAME in (select distinct s.PNAME from software s,studies st where DCOST=(select MIN(dcost)  from software))



Select Count(DEVELOPIN) as package From PROGRAMMER s, Software P
			where S.pname = (select p.pname where s.DCOST=MIN(s.dcost) )
			--and DCOST = (select min(DCOST) From Software)
			Group by DEVELOPIN having min(dcost)=(select min(dcost) from software)
...
select  count(*) as Countp from software s,programmer p
         ...
         where p.pname=s.pname and st.pname=p.pname and (st.INSTITUTE='SABHARI'));


		 Select Count(*) as Package_Count from Software s , Programmer p where p.PNAME=s.PNAME group by p.DOJ having max(p.DOJ) = (Select max(DOJ) from Programmer p , Studies st where 
p.PNAME=st.PNAME and st.INSTITUTE ='SABHARI')
...
SELECT *FROM programmer
create table new_t2 (name varchar(20),prof varchar(20),pkg_devlped int)

select * from new_t2
select pname,prof1 from programmer  
insert into new_t2 select pname,prof1,0 from programmer
insert into new_t2 select pname,prof2,0 from programmer
update new_t2 set pkg_devlped=(select COUNT(software.DEVELOPIN) as counter from software group by software.DEVELOPIN) where prof=software.DEVELOPIN


select developin,COUNT(DEVELOPIN) as pkg_dev from software group by DEVELOPIN
select * from software
select prof,count(*) as skill_counter from new_tab group by prof 
select developin,COUNT(DEVELOPIN) as pkg_dev from software group by DEVELOPIN

alter table new_tab drop pkg_developed
...
select distincta.pname,a.prof1,B.pname.b.prof2 as programmer a,programmer b

select programmer.PNAME,programmer.PROF1 from programmer full join programmer on programmer.PROF1<>programmer.PROF2

Select distinct(a.Pname), a. PROF2,b.PROF1 From PROGRAMMER a, PROGRAMMER b group by 
		                           where a.PROF1 <> b.PROF2 and a.pname <> b.pname
								   select* from programmer
...
SELECT *FROM software order by PNAME
SELECT *FROM programmer
